...
class Notifier( SubscriberBot ):
    ...
    def getNewSubmissions( self, user ):
        ...
        for post in redditor.get_submitted(sort='new', time='all', limit=10):
            ...
            if post.id != userData[self.CONSTANTS["DATABASE_COLUMNS"]["users"]["lastSubmissionId"]] and ( post.created > userData[self.CONSTANTS["DATABASE_COLUMNS"]["users"]["lastSubmissionDate"]] ):
    ...
    def getNewComments( self, user ):
        ...
        for post in redditor.get_comments(sort='new', time='all', limit=10):
            ...
            # If the user deletes their post, don't want to send a bunch of notifications
            if post.id != userData[self.CONSTANTS["DATABASE_COLUMNS"]["users"]["lastCommentId"]] and ( post.created > userData[self.CONSTANTS["DATABASE_COLUMNS"]["users"]["lastCommentDate"]] ):
    ...
    def notifyNewSubmission( self, post, user, subscriber ):
        ...
        for attempt in range(10):
            try:
                ...
                try: 
                    self.reddit.send_message(subscriber[self.CONSTANTS["DATABASE_COLUMNS"]["subscriber"]["subscriber"]], postSubject, postContent )
                except:
    ...
    def notifyNewComment( self, comment, user, subscriber ):
        ...
        for attempt in range(10):
            try:
                ...
                postContent = "Hi! User /u/{} has posted a new [comment]({}) on thread: [{}]({})".format(user, commentInfo.permalink,parentInfo.title, parentInfo.permalink)
                ...
                try:
                    ...
                    if newSubmissions or newComments:
                        ...
                        for subscriber in self.getSubscribers( user ):
                            if subscriber[self.CONSTANTS["DATABASE_COLUMNS"]["subscriber"]["subscriptionType"]] & self.CONSTANTS["SUBSCRIPTION_TYPE"]["submissions"] > 0:
                                for post in newSubmissions:
                                    self.notifyNewSubmission( post, user, subscriber )
                            if subscriber[self.CONSTANTS["DATABASE_COLUMNS"]["subscriber"]["subscriptionType"]] & self.CONSTANTS["SUBSCRIPTION_TYPE"]["comments"] > 0:
